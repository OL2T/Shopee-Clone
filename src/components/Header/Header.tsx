...
export default function Header() {
  ...
  const { data: cartData } = useQuery({
    ...
    enabled: isAuthenticated && !isPageCart
  })
        ...
        <div className='flex items-center flex-wrap justify-between'>
          ...
          >
            <div className='flex items-end'>
              ...
              {isPageCart && (
                <div className='border-l border-orange pl-[15px] ml-[15px] text-xl h-[30px] mb-[1px]'>
                  Giỏ hàng
                </div>
              )}
            </div>
          ...
          <div
            className={`${isPageCart ? 'max-w-[621px]' : 'flex-1 max-w-[840px]'} w-full`}
          >
            <form
              role='search'
              autoComplete='off'
              className={`${isPageCart ? 'border-2 border-orange max-w-[620px]' : 'p-1 rounded-sm w-full'} flex h-10 bg-white`}
            ...
            >
              <div className='w-full h-full'>
                <div className='relative'>
                  <input
                    ...
                    className={`${isPageCart ? 'max-w-[550px]' : ''} w-full h-full p-2 text-black outline-none`}
                    maxLength={128}
              ...
              <button
                type='submit'
                className={`${isPageCart ? 'px-8' : 'rounded-sm px-6'} btn btn-solid-primary relative z-1 bg-orange flex items-center justify-center  py-2 `}
              >
                <svg
                  height={19}
                  viewBox='0 0 19 19'
